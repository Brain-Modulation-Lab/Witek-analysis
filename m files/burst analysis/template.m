...
% TEMPLATE Application M-file for Burst.fig
%    FIG = TEMPLATE launch Burst GUI.
%    TEMPLATE('callback_name', ...) invoke the named callback.

% Last Modified by GUIDE v2.0 09-May-2003 12:29:41
...
function varargout = DiscriminateButton_Callback(h, eventdata, handles, varargin)
    ...
    disp('Discriminating...');
    handles.D = discriminate(handles.H, str2num(get(handles.Threshold, 'String')), get(handles.PosRadio, 'Value'));
	guidata(handles.figure, handles);
...
function varargout = FindBurstsButton_Callback(h, eventdata, handles, varargin)
    ...
    if s(1) > 0
        ...
        axes(handles.Axes);
        xlim([handles.B(handles.CurrentBurst, 1)-50 handles.B(handles.CurrentBurst, 1)+handles.B(handles.CurrentBurst, 2)+50]);
