function [Pbase_mean, Ptest_mean, Pbase_sterr, Ptest_sterr] = pairdata_analysis(bdel, tdel, base, test, nbin)
...
Pbase_mean = mean(Pbase);
Pbase_sterr = std(Pbase)/sqrt(size(base,1));
Ptest_mean = mean(Ptest);
Ptest_sterr = std(Ptest)/sqrt(size(test,1));