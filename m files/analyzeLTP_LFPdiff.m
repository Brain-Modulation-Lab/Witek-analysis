...
for i = 1:length(LTP_LFPanalysis)
    if strcmp(LTP_LFPanalysis(i).group, 'Y')
        ...
        y=y+1;
    elseif strcmp(LTP_LFPanalysis(i).group, 'NAIVE')
...
xlim(xlimits);
ylim(ylimits);
title([TITLE, ' difference from baseline (0 - ', num2str(ceil(T3/6)), ' min)']);
...
xlim(xlimits);
ylim(ylimits);
title([TITLE, ' difference from baseline (', num2str(ceil((T1+1)/6)), ' - ', num2str(ceil((T2+1)/6)), ' min)']);
...
xlim(xlimits);
ylim(ylimits);
title([TITLE, ' difference from baseline (', num2str(ceil((T2+1)/6)), ' - ', num2str(ceil((T3)/6)), ' min)']);