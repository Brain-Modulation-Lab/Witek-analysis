function LTP_LFPanalysis = group_analyzeLTP_LFPmax(LFPdata)
...
%     
    ...
    [base_j, test_j] = analyzeLTP_LFPmax([year, '/', LFPdata{j,2}, ' MAT', '/', LFPdata{j,2}, LFPdata{j,3}], {[year, '/', LFPdata{j,2}, ' MAT', '/', LFPdata{j,2}, LFPdata{j,4}]}, tbase, tstart, tstop, 0, 1, sf, stimf, pre);
...
%     end
    ...
    LTP_LFPanalysis(j).base.max = base_j.max;
    %LTP_LFPanalysis(j).base.tmin = base_j.tmin;
    LTP_LFPanalysis(j).base.tmax = base_j.tmax;
    ...
    LTP_LFPanalysis(j).test.max = test_j.max;
    %LTP_LFPanalysis(j).test.tmin = test_j.tmin;
    LTP_LFPanalysis(j).test.tmax = test_j.tmax;
