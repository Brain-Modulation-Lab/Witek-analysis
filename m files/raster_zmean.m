function [t, Nbin, zz, h] = raster_zmean(DD, dur, sf, epoch_num, pre, epoch_length, stim_dur, nbin, filename)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Z-SCORE PANEL -- mean z-score per trial
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1:epoch_num
    ...
    sigma = std(NNbin(1:prebins,i));
    for j=1:nbin
        zz(j,i) = (NNbin(j,i)-mean(NNbin(1:prebins,i)))/sigma;
    end
    %     H = kstest(zz(:,i));
    %     [Hzz, zz0] = hist(zz(:,i), 10);
    %     figure; bar(zz0, Hzz); title(num2str(H));
    %ZZ(end+1:end+size(NNbin(1:prebins,i),1)) = NNbin(1:prebins,i);
end
% ZZ
% H = kstest(ZZ(:,i))
% [Hzz, zz0] = hist(ZZ, 50);
