...
for i=1:length(dat_files)
    ...
    if isfield(SS, 'unitsL')
        ...
        VL_hfa_ipsi_indiv(cell2mat(SS.al))=squeeze((mean(HFAWl(:,2,:),1)-mean(HFAWl(:,1,:),1)))';
        ...
        VL_hfa_contra_indiv(cell2mat(SS.al))=squeeze((mean(HFAWr(:,2,:),1)-mean(HFAWr(:,1,:),1)))';
    ...
    if isfield(SS, 'unitsR')
        ...
        VR_hfa_ipsi_indiv = zeros(size(SS.VR_indiv));
        %VR_hfa_ipsi_indiv(cell2mat(SS.ar))=-log(p_ipsi).*h_ipsi;
        VR_hfa_ipsi_indiv(cell2mat(SS.ar))=squeeze((mean(HFAWr(:,2,:),1)-mean(HFAWr(:,1,:),1)))';
        V_hfa_ipsi_comm = V_hfa_ipsi_comm + SS.Wmat*VR_hfa_ipsi_indiv;
        
        VR_hfa_contra_indiv = zeros(size(SS.VR_indiv));
        %VR_hfa_contra_indiv(cell2mat(SS.ar))=-log(p_contra).*h_contra;
        VR_hfa_contra_indiv(cell2mat(SS.ar))=squeeze((mean(HFAWl(:,2,:),1)-mean(HFAWl(:,1,:),1)))';
        V_hfa_contra_comm = V_hfa_contra_comm + SS.Wmat*VR_hfa_contra_indiv;
...
colormap jet
Clim = get(gca, 'clim');
colorbar('Ticks', [Clim(1) Clim(2)], 'TickLabels', {num2str(min(V_hfa_contra_comm)), num2str(max(V_hfa_contra_comm))})
title('CONTRA')