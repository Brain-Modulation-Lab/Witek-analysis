...
%left
    if exist([FS_subject_path,subjects{s},'_FS/Electrode_Locations/FinalLoc/CortElecL.mat'])
       ...
       for i=1:length(al)
           ...
           Vtemp = Wmat*Vtemp;
           nzidx = find(Vtemp~=0);
           ElecLocCM{i} = (Vertices(nzidx,:)'*(Vtemp(nzidx)/sum(Vtemp(nzidx))))';
       end
        
        save([output_path,subjects{s}], 'CortElecLocL', 'ElecLocCM', 'VL_indiv', 'VL_comm', 'al', '-append');
...
%         ElecVertsR = cellfun(@(A,B) B(A,:), al, repmat({cortex.vert}, size(al,1),1), 'uniformoutput', false )';
%         VR_indiv = zeros(length(cortex.vert),1);
%         VR_indiv(cell2mat(ar))=1;
%         VR_comm = Wmat*VR_indiv;
%         
%         save([output_path,subjects{s}], 'CortElecLocR', 'VR_indiv', 'VR_comm', 'ar', '-append');
