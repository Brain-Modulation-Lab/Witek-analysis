function [ mi, zmi, pmi, phase_hist, FDR, Q ] = miSPL_AA( LFP, tstamps, fq, nbin, niter )
...
parfor f=1:length(fq)
    ...
    sph=int8(sph);
    Psurr=single(genSTAsurr(sph,new_tstamps,prespike,postspike,niter));
...
function Psurr=genSTAsurr(sph,new_tstamps,prespike,postspike,niter)
phSTAsurr=zeros(1001,niter,length(new_tstamps),'int8');
for j = 1:length(new_tstamps)
 phSTAsurr(:,:,j)=sph(new_tstamps(j)-prespike:new_tstamps(j)+postspike,:);      
end
  phSTAsurr=permute(phSTAsurr,[3,1,2]);
  Psurr = histc(phSTAsurr,1:30,1);
end