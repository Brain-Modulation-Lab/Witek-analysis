...
for i=1:length(dat_files)
    ...
    for trial=1:length(S.QLeftResponseTimes)
        idx = find(S.EventTimes < S.QLeftResponseTimes(trial)/S.fs,1,'last');
        if idx>3
        ...
        SPLtrial.Left(trial).epoch(5) = S.QLeftResponseTimes(trial)/S.fs;
        [Fmax, ~] = max(S.Force(round(S.QLeftResponseTimes(trial)):round(S.QLeftResponseTimes(trial)+MaxGripTime*S.fs),1));
        SPLtrial.Left(trial).epoch(6) = S.QLeftResponseTimes(trial)/S.fs + ...
        ...
        idx = find(S.EventTimes > S.QLeftResponseTimes(trial)/S.fs,1,'first');
        ...
        [Fmax, Fmax_idx] = max(S.Force(round(S.QLeftResponseTimes(trial)):round(SPLtrial.Left(trial).epoch(6)*S.fs),1));
        Psychometrics(i).Left(trial).Fmax = Fmax - S.Force(round(S.QLeftResponseTimes(trial)),1);
    ...
    for trial=1:length(S.QRightResponseTimes)
        idx = find(S.EventTimes < S.QRightResponseTimes(trial)/S.fs,1,'last');
        if idx>3
        ...
        SPLtrial.Right(trial).epoch(5) = S.QRightResponseTimes(trial)/S.fs;
        [Fmax, ~] = max(S.Force(round(S.QRightResponseTimes(trial)):round(S.QRightResponseTimes(trial)+MaxGripTime*S.fs),2));
        SPLtrial.Right(trial).epoch(6) = S.QRightResponseTimes(trial)/S.fs + ...
        ...
        idx = find(S.EventTimes > S.QRightResponseTimes(trial)/S.fs,1,'first');
        if idx<=length(S.EventTimes)
        ...
        [Fmax, Fmax_idx] = max(S.Force(round(S.QRightResponseTimes(trial)):round(SPLtrial.Right(trial).epoch(6)*S.fs),2));
        Psychometrics(i).Right(trial).Fmax = Fmax - S.Force(round(S.QRightResponseTimes(trial)),2);
