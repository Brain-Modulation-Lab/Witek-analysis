...
%         VL(aconn) = VL(aconn)+HiB_L(t,v)*exp(-dconn/tau)';
%         VR(aconn) = VR(aconn)+HiB_R(t,v)*exp(-dconn/tau)';
        VL(aconn) = VL(aconn)+1*exp(-dconn/tau)';
        VR(aconn) = VR(aconn)+0.5*exp(-dconn/tau)';
    end
...
%         h_text = text(0, 0, num2str(10*(t-1)/200-5), 'FontSize', 16);
        ...
        HpL = patch('vertices',subject.Vertices,'faces',subject.Faces,'FaceVertexCData', VL,'edgecolor','none','FaceColor','interp');
        ...
        HpR = patch('vertices',subject.Vertices,'faces',subject.Faces,'FaceVertexCData', VR,'edgecolor','none','FaceColor','interp');
...
%         xlim([0 10001]); ylim([-10 510]); hold on;
        ...
        set(HpL,'FaceVertexCData',VL);
        set(HpR,'FaceVertexCData',VR);
        M(t) = getframe(gcf);
    end
    pause(0.01)
end