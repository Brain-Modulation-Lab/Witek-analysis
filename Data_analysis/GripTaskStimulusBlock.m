function [StimulusTimes, StimulusDuration, Trial, TrialNum] = GripTaskStimulusBlock(EventTimes, SkipEvents, EventsPerTrial, Event, TaskDataLog)
...
% Trial.Go = find([TaskDataLog{:,9}]==1);
% Trial.NoGo = find([TaskDataLog{:,9}]==2);
% Trial.FastGo = find([TaskDataLog{:,9}]==3);
% 
% % find correct and error trials
% Trial.Correct = find([TaskDataLog{:,10}]==1);
% Trial.Error = find([TaskDataLog{:,10}]==0);

% find go and nogo trials
Trial.Go = find([TaskDataLog{:,11}]==1);
Trial.NoGo = find([TaskDataLog{:,11}]==2);
Trial.FastGo = find([TaskDataLog{:,11}]==3);
